...
<html>
	<head>
		...
		<script>
			...
			var webSocket = new WebSocket("ws://" + host + ":8080/WordGame"); // 192.168.0.84
			...
			function disconnect(){
				if (webSocket && webSocket.readyState === WebSocket.OPEN) {
			        webSocket.close();  // 연결 닫기 시도
			    }
				setTimeout(() => {
				        window.close();    // 약간의 딜레이 후 창 닫기 (보안상 팝업이어야 함)
				    }, 100);
			}
			function enterKey(){
			...
			webSocket.onopen = function(event){
				...
				chatWindow.innerHTML += "끝말잇기 방에 입장하셨습니다.<br/>";
				webSocket.send("JOIN|wordgame|" + nickname);
			};
			
			webSocket.onclose = function(event){
				chatWindow.innerHTML += "끝말잇기가 종료되었습니다.<br/>";
			};
